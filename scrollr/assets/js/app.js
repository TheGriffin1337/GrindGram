...
let state = {
    ...
    loading:  false,
    hasMore:  true,
    view:     'feed',   // feed | upload | ai | profile
};
...
function showView(name) {
    ...
    // Dodatkowe akcje przy przejściu
    if (name === 'feed' && state.posts.length === 0) loadFeed();
    if (name === 'profile') renderProfile();

    // Chowaj/pokazuj nav
    const topNav = document.getElementById('top-nav');
    topNav.style.display = name === 'feed' ? 'flex' : name === 'profile' ? 'none' : 'flex';
}
...
function renderFeed(reset = false) {
    ...
    const newPosts = state.posts.slice(reset ? 0 : state.posts.length - 5);
    ...
    if (newPosts.length === 0 && reset) {
        feed.innerHTML = `
            <div class="empty">
                ...
                <h3>Brak postów</h3>
                <p>Dodaj pierwszy post lub wygeneruj go przez AI!</p>
            </div>`;
        return;
    }
...
function createPostCard(post) {
    ...
    // Dekoracyjne koła (tylko text posty)
    if (!hasMedia) {
...
async function toggleLike(btn, postId) {
    if (!state.user) { toast('Zaloguj się aby polubić', 'error'); return; }
    ...
    try {
        ...
        count.textContent = liked ? cur + 1 : Math.max(0, cur - 1);
...
async function sharePost(postId) {
    const url = `${location.origin}/scrollr/?post=${postId}`;
    if (navigator.share) {
...
function initUploadForm() {
    ...
    function showPreview(file) {
        ...
        if (file.type.startsWith('image/')) {
            preview.innerHTML = `<img src="${url}" style="max-width:100%;border-radius:10px;margin-top:12px">`;
        } else {
            preview.innerHTML = `<video src="${url}" controls style="max-width:100%;border-radius:10px;margin-top:12px"></video>`;
        }
    }

    form.addEventListener('submit', async e => {
        e.preventDefault();
        if (!state.user) { toast('Musisz być zalogowany', 'error'); return; }
        ...
        <div style="padding:20px">
            <div class="profile-header">
                ...
                <div>
                    ...
                    <div class="text-muted">Członek Scrollr</div>
                </div>
            </div>
...
async function boot() {
    ...
    // Poczekaj chwilę zanim załadujesz feed (bez logowania)
    if (state.token) {
        try {
            ...
            state.token = null;
            localStorage.removeItem('scrollr_token');
            showMainApp(); // Pokaż feed bez zalogowania
        }
    } else {
        showMainApp(); // Gość też widzi feed
    }

    // Service Worker
    if ('serviceWorker' in navigator) {
        navigator.serviceWorker.register('/scrollr/sw.js').catch(() => {});
    }
}
